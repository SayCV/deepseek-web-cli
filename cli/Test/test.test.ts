...
import {
  ...
  ToolRegistry, ToolExecutor, extractToolCall,
} from "../chat.ts"
...
test("extractToolCall 解析 fenced tool_json 格式", () => {
  const call = extractToolCall('```tool_json\n{"tool":"exec","parameters":{"command":"pwd"}}\n```')
  assert.deepEqual(call, { tool: "exec", parameters: { command: "pwd" } })
})

test("extractToolCall 解析裸 JSON 格式", () => {
  const call = extractToolCall('{"tool":"read","parameters":{"path":"README.md"}}')
...
test("ToolRegistry.buildToolPrompt 包含 tool_json 示例和工具参数", () => {
  ...
  assert.match(prompt, /```tool_json/)
  assert.match(prompt, /plus_one/)
  assert.match(prompt, /"name":"exec"/)
  assert.match(prompt, /"command":"string"/)
})

test("ToolExecutor 拒绝读取 workspace 外文件", async () => {
  const workspace = await fs.mkdtemp(path.join(TMP, "tool-workspace-"))
  const outside = await fs.mkdtemp(path.join(TMP, "tool-outside-"))
  await fs.writeFile(path.join(outside, "secret.txt"), "secret")
  const registry = new ToolRegistry()
  registry.register({ name: "read", description: "Read file", parameters: { path: { type: "string", description: "File path" } } })
  const executor = new ToolExecutor(registry, workspace)

  const result = await executor.execute("read", { path: path.join(outside, "secret.txt") })
...
test("ToolExecutor write 相对路径写入 workspace 内", async () => {
  const workspace = await fs.mkdtemp(path.join(TMP, "tool-workspace-"))
  const registry = new ToolRegistry()
  registry.register({
    name: "write",
  ...
  const executor = new ToolExecutor(registry, workspace)
  const yesRl = { question: (_q: string, cb: (answer: string) => void) => cb("y") } as any

  const result = await executor.execute("write", { path: "nested/out.txt", content: "hello" }, yesRl)

  assert.equal(result, "success")
  assert.equal(await fs.readFile(path.join(workspace, "nested", "out.txt"), "utf-8"), "hello")
  await fs.rm(workspace, { recursive: true, force: true })
})

test("ToolExecutor exec 在 workspace cwd 执行", async () => {
  const workspace = await fs.mkdtemp(path.join(TMP, "tool-workspace-"))
  const registry = new ToolRegistry()
  registry.register({ name: "exec", description: "Run command", parameters: { command: { type: "string", description: "Command" } } })
  const executor = new ToolExecutor(registry, workspace)
  const yesRl = { question: (_q: string, cb: (answer: string) => void) => cb("y") } as any

  const result = await executor.execute("exec", { command: "pwd" }, yesRl)

  assert.equal(path.resolve(result.trim()), path.resolve(workspace))
  await fs.rm(workspace, { recursive: true, force: true })
})

test("ToolExecutor web_fetch 抓取 URL 文本", async () => {
  ...
  const registry = new ToolRegistry()
  registry.register({ name: "web_fetch", description: "Fetch URL", parameters: { url: { type: "string", description: "URL" } } })
  const executor = new ToolExecutor(registry)

  try {
    const result = await executor.execute("web_fetch", { url: `http://127.0.0.1:${address.port}/hello` })
    assert.equal(result, "hello from local server")
  } finally {
    await new Promise<void>((resolve, reject) => server.close((err) => err ? reject(err) : resolve()))
  }
})

test("ToolExecutor web_fetch 拒绝非 HTTP URL", async () => {
  const registry = new ToolRegistry()
  registry.register({ name: "web_fetch", description: "Fetch URL", parameters: { url: { type: "string", description: "URL" } } })
  const executor = new ToolExecutor(registry)

  const result = await executor.execute("web_fetch", { url: "file:///etc/passwd" })

  assert.match(result, /Only http and https URLs are allowed/)
})
