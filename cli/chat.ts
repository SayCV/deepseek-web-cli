...
function extractToolCall(text: string): ParsedToolCall | null {
  const fenced = FENCED_TOOL_JSON_REGEX.exec(text);
  if (fenced) return parseToolJson(fenced[1]);

  const bare = BARE_TOOL_JSON_REGEX.exec(text);
  if (bare) {
    try {
...
class DeepSeekClient {
  ...
  private async fetchFileStatus(fileId: string): Promise<{ status: string } | null> {
  ...
  async waitForUploadedFile(fileId: string, maxAttempts = 60): Promise<void> {
    const readyStatuses = ["processed", "ready", "done", "available", "success", "completed", "finished"];
...
class StreamParser {
  ...
  private processData(data: any) {
    ...
    const fullToolCall = extractToolCall(content);
    if (fullToolCall) {
      this.events.push({
        type: "tool_call_start",
        id: "",
        name: fullToolCall.tool,
      });
      this.events.push({
        type: "tool_call_end",
        id: "",
        name: fullToolCall.tool,
        arguments: fullToolCall.parameters,
      });
      return;
    }
  ...
  private checkTags() {
    ...
    if (indices.length === 0) {
      const looksLikePartialTag = /^<[^>]*$/.test(this.tagBuffer.trimStart());
      const isInTagStream = this.textTagDepth > 0 || this.currentMode === "tool_call";
      if (!looksLikePartialTag && !isInTagStream && this.tagBuffer) {
  ...
  buildToolPrompt(): string {
    ...
    const compactTools = tools.map((tool) => ({
      ...
      parameters: Object.fromEntries(
        Object.entries(tool.parameters).map(([key, value]) => [key, value.type]),
      ),
    }));
    return [
      "## Available Tools",
      ...
      "需要调用真实工具时，只回复 tool_json 代码块，不要附加说明。不需要工具则直接回答。",
    ].join("\n");
  }
}
...
class ToolExecutor {
  ...
  private resolveWorkspacePath(input: unknown): string {
    if (typeof input !== "string" || !input.trim()) {
      throw new Error("Invalid path");
    }
    const resolved = path.resolve(this.workDir, input);
    const root = path.resolve(this.workDir);
    const relative = path.relative(root, resolved);
    if (relative.startsWith("..") || path.isAbsolute(relative)) {
      throw new Error(`Access denied outside workspace: ${input}`);
    }
    return resolved;
  }
  ...
  async execute(name: string, args: Record<string, unknown>, rl?: readline.Interface): Promise<string> {
    ...
    try {
      if (name === "web_fetch") return await this.webFetch(args.url);
    ...
    try {
      switch (name) {
        case "read": {
          ...
          return annotateReadOutput(raw, filePath);
        }
        case "write": {
          const filePath = this.resolveWorkspacePath(args.path);
  ...
  private async webFetch(urlInput: unknown): Promise<string> {
...
class ChatSession {
  ...
  async *send(userMessage: string, rawMode: boolean): AsyncGenerator<ParseEvent> {
    ...
    // 检测空回复（房间可能已被删除）
    if (!assistantText && !foundToolCall && !hadError) {
      ...
      this.state.updatedAt = Date.now();
      this.save();
      yield { type: "error", message: "服务端已回复，但 assistantText 内容为空，房间可能已被删除" };
  ...
  switch (cmd) {
    ...
    case "up": case "upload": {
      ...
      try {
        ...
        log(`⏳ 等待服务器就绪...`);
        await ctx.client.waitForUploadedFile(fileId);
        log(`✅ 文件已就绪`);
...
function displayEvent(event: ParseEvent, rawMode: boolean, thinkingActive: boolean): boolean {
  ...
  if (rawMode) {
    ...
    else if (event.type === "clear_response") console.log(`\n⚠️ 服务端请求清除之前的回复文本（已被本地保留）`);
...
function switchWorkspace(
  ...
  ctx.toolExecutor.setWorkDir(newDir);
